...
- [x] Все CI job’ы используют `npm ci` (шаг Install dependencies обновлён).
- [x] CI успешно проходит после изменения (workflow `main` от 2025-11-15 завершился успехом с `npm ci` + реструктурированными шагами).
...
- [x] Pre-commit хук установлен и запускается при `git commit` (Husky `prepare` + `.husky/pre-commit`).
- [x] Нельзя закоммитить неотформатированный/поломанный код (`lint-staged` гоняет `eslint --fix`/`prettier --write`).
