...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!pendingMetadata?.puppy_id) {
      return NextResponse.json({ error: 'Missing puppy metadata' }, { status: 400 });
    }

    const hasActiveReservation = await ReservationQueries.hasActiveReservation(
      pendingMetadata.puppy_id,
    );
    ...
    if (!capture || !capture.id) {
      return NextResponse.json({ error: 'Capture details not found in response' }, { status: 500 });
    }
